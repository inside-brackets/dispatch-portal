...
mongoose
  .connect(
    process.env.PROD === 'true'? process.env.FALCON_DB:process.env.FALCON_DB_TEST,
    {
  ...
  .then((result) => {
    console.log(process.env.PROD === 'true'? "Real database connected":"Test database connected");
  })
  .catch((err) => {
    throw err;
  });
